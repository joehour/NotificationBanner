...
open class NotificationBannerView: UIView {
    ...
    open var currentStyle: BannerStyle = .sucessMessage
    ...
    @objc
    public enum Location: NSInteger {
        case bottom
        case top
    }
    
    @objc
    public enum BannerStyle: NSInteger {
        case customView
        case errorMessage
        case sucessMessage
        case infoMessage
    ...
    open func setParameter(_ textContent: String = "", textColor: UIColor = .white, backgroundColor: UIColor = .black) {
    ...
    internal func dissmiss(_ completion: @escaping (_ Sucess: Bool)->()) {
        dismisssing = true
        UIView.animate(
            withDuration: Double(0.5), delay: 0, options: [.curveEaseInOut, .allowUserInteraction],
            animations: {
    ...
    internal func rotate() {
        if currentDevice.orientation != orientation {
            ...
            setContentPosition(fixHeight, setStatusBarHeight: fixStatusBarHeight)
            orientation = currentDevice.orientation
        }
    }
    
    internal func setContentPosition(_ setHight: Int, setStatusBarHeight: Int ) {
    ...
    internal func changeStyle(_ style: BannerStyle, bannerBackgroundColor: UIColor = .blue, bannerImage image: UIImage = UIImage(contentsOfFile: Bundle(for: NotificationBannerView.self).path(forResource: "ic_info_white_3x", ofType: "png")!)!) {
        currentStyle = style
        switch style {
        case .errorMessage:
            self.backgroundColor = .red
    ...
    internal func setText(_ textTitle: String = "", textContent: String = "") {
        titleView.text = textTitle
        textView.text = textContent
    }
