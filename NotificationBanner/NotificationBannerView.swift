...
open class NotificationBannerView : UIView{
    ...
    internal func rotate(){
        ...
        if currentDevice.orientation != orientation {
            ...
            //var diffValue: CGFloat = self.currentView.bounds.height - self.currentView.bounds.width
            ...
            setContentPosition(_fixtHight: fixHeight, _fixStatusBarHeight: fixStatusBarHeight)
            orientation = currentDevice.orientation
            
        }
    }
    
    internal func setContentPosition(_fixtHight: Int , _fixStatusBarHeight:Int ){
        
        self.cancelView.frame = (frame: CGRect(x: Int(view.bounds.width - self.currentHeight*0.5), y: Int(self.currentHeight*0.17) + _fixtHight , width: Int(self.currentHeight*0.55), height: Int(self.currentHeight*0.55)))
        
        
        self.titleView.frame = (frame: CGRect(x: 20 + Int(self.currentHeight*0.5), y: 0 + _fixtHight + _fixStatusBarHeight, width: Int(view.bounds.width - self.currentHeight - 30), height: Int(self.currentHeight*0.45)))
        
        self.textView.frame = (frame: CGRect(x: 20 + Int(self.currentHeight*0.5), y: Int(self.currentHeight*0.33) + _fixtHight + _fixStatusBarHeight, width: Int(view.bounds.width - self.currentHeight - 30), height: Int(self.currentHeight*0.35)))
    }
    ...
    internal func setBannerContent(style: Style, textTitle: String = "", textContent : String = "", textColor: UIColor = .white, bannerLocation location: Location = .Top, messageTitleFont titleFont: CGFloat = 25, messageContentFont contentFont: CGFloat = 15, bannerHoldTime holdTime: Int = 5, bannerBackgroundColor: UIColor = .blue, bannerImage image: UIImage = UIImage(contentsOfFile: Bundle(for: NotificationBannerView.self).path(forResource: "ic_info_white_3x", ofType: "png")!)!) {
        ...
        titleView.contentInset = UIEdgeInsetsMake(-CGFloat(currentHeight*0.08),0.0,0,0.0);
        ...
        textView.contentInset = UIEdgeInsetsMake(-CGFloat(currentHeight*0.08),0.0,0,0.0);
